...
40000003:	53                   	push   %ebx
    printf("ping started.\n");
40000004:	bb 0a 00 00 00       	mov    $0xa,%ebx
#include <stdio.h>
#include <syscall.h>
#include <x86.h>
...
int main (int argc, char **argv)
{
40000009:	83 e4 f0             	and    $0xfffffff0,%esp
4000000c:	83 ec 10             	sub    $0x10,%esp
    printf("ping started.\n");
4000000f:	c7 04 24 dc 12 00 40 	movl   $0x400012dc,(%esp)
40000016:	e8 35 02 00 00       	call   40000250 <printf>
4000001b:	90                   	nop
4000001c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

    unsigned int i;
    //fast producing
    for (i = 0; i < 10; i++)
      produce();
40000020:	e8 0b 09 00 00       	call   40000930 <produce>
{
    printf("ping started.\n");
    ...
    //fast producing
    for (i = 0; i < 10; i++)
40000025:	83 eb 01             	sub    $0x1,%ebx
40000028:	75 f6                	jne    40000020 <main+0x20>
4000002a:	31 db                	xor    %ebx,%ebx
4000002c:	eb 0a                	jmp    40000038 <main+0x38>
4000002e:	66 90                	xchg   %ax,%ax
      produce();

    //slow producing
    for (i = 0; i < 40; i++) {
40000030:	83 c3 01             	add    $0x1,%ebx
40000033:	83 fb 28             	cmp    $0x28,%ebx
40000036:	74 12                	je     4000004a <main+0x4a>
      if (i % 4 == 0)
40000038:	f6 c3 03             	test   $0x3,%bl
4000003b:	75 f3                	jne    40000030 <main+0x30>
    //fast producing
    for (i = 0; i < 10; i++)
      produce();
    ...
    //slow producing
    for (i = 0; i < 40; i++) {
4000003d:	83 c3 01             	add    $0x1,%ebx
      if (i % 4 == 0)
        produce();
40000040:	e8 eb 08 00 00       	call   40000930 <produce>
    //fast producing
    for (i = 0; i < 10; i++)
      produce();
    ...
    //slow producing
    for (i = 0; i < 40; i++) {
40000045:	83 fb 28             	cmp    $0x28,%ebx
40000048:	75 ee                	jne    40000038 <main+0x38>
      if (i % 4 == 0)
        produce();
    }
...
4000004a:	31 c0                	xor    %eax,%eax
4000004c:	8b 5d fc             	mov    -0x4(%ebp),%ebx
4000004f:	c9                   	leave  
40000050:	c3                   	ret    
...
40000051:	f7 c4 ff ff ff 0f    	test   $0xfffffff,%esp
	jnz	args_exist
40000057:	75 04                	jne    4000005d <args_exist>
...
noargs:
	...
	pushl	$0
40000059:	6a 00                	push   $0x0
	pushl	$0
4000005b:	6a 00                	push   $0x0
...
args_exist:
	/* Jump to the C part. */
	call	main
4000005d:	e8 9e ff ff ff       	call   40000000 <main>

	/* When returning, push the return value on the stack. */
	pushl	%eax
40000062:	50                   	push   %eax

40000063 <spin>:
spin:
	//call	yield
	jmp	spin
40000063:	eb fe                	jmp    40000063 <spin>
40000065:	66 90                	xchg   %ax,%ax
40000067:	66 90                	xchg   %ax,%ax
40000069:	66 90                	xchg   %ax,%ax
4000006b:	66 90                	xchg   %ax,%ax
4000006d:	66 90                	xchg   %ax,%ax
4000006f:	90                   	nop
